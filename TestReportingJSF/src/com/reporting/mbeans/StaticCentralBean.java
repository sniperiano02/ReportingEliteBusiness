...
import com.reporting.metier.interfaces.StatCentraleNatImplRemote;



@ManagedBean
@ViewScoped
public class StaticCentralBean implements Serializable {
	...
	@EJB
	StatCentraleNatImplRemote stat_remote;
	...
	private String choix_stat;
	 ...
	 private List<Object[]> staticListStatCentral = new ArrayList<>();
...
private List<String> listeCti= new ArrayList<String>();
...
chartDisplayed = false;
		...
		if(compJourOutputHeure!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				ValueExpression valjour = createValueExpression("#{staticCentralBean.date_ParJourDeb}", Date.class);
				...
				ValueExpression valjourfin = createValueExpression("#{staticCentralBean.date_ParJourFin}", Date.class);
			...
			ValueExpression valjour = createValueExpression("#{staticCentralBean.date_Parheure}", Date.class);
			...
			AjaxBehavior ajax = new AjaxBehavior();
		       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
		        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
		            createActionMethodExpression("#{staticCentralBean.handlechange1}"),createActionMethodExpression("#{staticCentralBean.handlechange1}")));
		        ajax.setUpdate("@form");
...
//			        
		         ...
		         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{staticCentralBean.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
				...
				lstDateYears.setId("lstDateYears");
				 ValueExpression valueExp = createValueExpression("#{staticCentralBean.date_year_deb}", Integer.class);
				...
				lstDateYearsFin.setId("lstDateYearsFin");
				 ValueExpression valueExp1 = createValueExpression("#{staticCentralBean.date_year_fin}", Integer.class);
			...
			ValueExpression valjour = createValueExpression("#{staticCentralBean.date_mois_debut}", Date.class);
			...
			ValueExpression valjourfin = createValueExpression("#{staticCentralBean.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		staticListStatCentral = new ArrayList<>();
...
// 			}
		...
		if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 staticListStatCentral = stat_remote.getAllStatCentralStatic("substring(trancheHoraire,1,2)", "SUM(dureeMoyenne),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);
		
	 		 }else if(this.getChoix_periode().equals("Par An")){
	 			staticListStatCentral = stat_remote.getAllStatCentralStatic("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(dureeMoyenne),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);
	 			
	 			
		 		 }else if(this.getChoix_periode().equals("Par Mois")){
		 			staticListStatCentral = stat_remote.getAllStatCentralStatic("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(dureeMoyenne),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			staticListStatCentral = stat_remote.getAllStatCentralStatic("to_char(to_date(dateAppel,'YYMMDD'),'DD')", "SUM(dureeMoyenne),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);
	...
	public List<String> getListeCti() {
		return listeCti;
	}
...
public void handlechange2(AjaxBehaviorEvent event){
	...
	public void setListeCti(List<String> listeCti) {
		this.listeCti = listeCti;
	}
	...
	public List<Object[]> getStaticListStatCentral() {
		return staticListStatCentral;
	}
	public void setStaticListStatCentral(List<Object[]> staticListStatCentral) {
		this.staticListStatCentral = staticListStatCentral;
	}

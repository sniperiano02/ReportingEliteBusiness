...
@ManagedBean(name="balance_neg")
@ViewScoped
public class Balance_NegMbean {

	
	@EJB
	private Balance_NegRemote balance_service;
	
	
	 private List<Object[]> stat_balance_neg= new ArrayList<>();
			...
			chartDisplayed = false;
					...
					if(compJourOutputHeure!=null){
						...
						if(this.getChoix_periode().equals("Par Jour")){
							...
							ValueExpression valjour = createValueExpression("#{balance_neg.date_ParJourDeb}", Date.class);
						...
						addAjax(cld_jour_debut);
							...
							ValueExpression valjourfin = createValueExpression("#{balance_neg.date_ParJourFin}", Date.class);
						...
						ValueExpression valjour = createValueExpression("#{balance_neg.date_Parheure}", Date.class);
						...
						AjaxBehavior ajax = new AjaxBehavior();
					       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
					        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
					            createActionMethodExpression("#{balance_neg.handlechange1}"),createActionMethodExpression("#{balance_neg.handlechange1}")));
					        ajax.setUpdate("@form");
...
//						        
					         ...
					         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{balance_neg.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
							...
							lstDateYears.setId("lstDateYears");
							 ValueExpression valueExp = createValueExpression("#{balance_neg.date_year_deb}", Integer.class);
							...
							lstDateYearsFin.setId("lstDateYearsFin");
							 ValueExpression valueExp1 = createValueExpression("#{balance_neg.date_year_fin}", Integer.class);
						...
						ValueExpression valjour = createValueExpression("#{balance_neg.date_mois_debut}", Date.class);
						...
						ValueExpression valjourfin = createValueExpression("#{balance_neg.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		stat_balance_neg = new ArrayList<>();
	 ...
	 if(this.getChoix_periode().equals("Par An")){
						stat_balance_neg = balance_service.getAllStatBalance("Extract (year from to_date(dateAppel,'YYMMDD')) ","SUM(impact),SUM(nbAppel)" , "", "", where_liste);

		 		 }else if(this.getChoix_periode().equals("Par Mois")){
						stat_balance_neg = removeDuplicate(generateList,balance_service.getAllStatBalance("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')","SUM(impact),SUM(nbAppel)" , "", "", where_liste));

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
						stat_balance_neg = removeDuplicate(generateList, balance_service.getAllStatBalance("to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD')","SUM(impact),SUM(nbAppel)" , "", "", where_liste));
...
//			        
	...
	public List<Object[]> getStat_balance_neg() {
		return stat_balance_neg;
	}
	public void setStat_balance_neg(List<Object[]> stat_balance_neg) {
		this.stat_balance_neg = stat_balance_neg;
	}

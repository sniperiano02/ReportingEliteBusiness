...
@ManagedBean(name="stat_tap_in")
@ViewScoped
public class StatTapINMbean {

	
	
	@EJB
	private StatTapINRemote stat_tap_in_service;
...
public void init(){
		...
		liste_desAns=stat_tap_in_service.getAllYears();
...
chartDisplayed = false;
		...
		if(compJourOutputHeure!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				ValueExpression valjour = createValueExpression("#{stat_tap_in.date_ParJourDeb}", Date.class);
			...
			addAjax(cld_jour_debut);
				...
				ValueExpression valjourfin = createValueExpression("#{stat_tap_in.date_ParJourFin}", Date.class);
			...
			ValueExpression valjour = createValueExpression("#{stat_tap_in.date_Parheure}", Date.class);
			...
			AjaxBehavior ajax = new AjaxBehavior();
		       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
		        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
		            createActionMethodExpression("#{stat_tap_in.handlechange1}"),createActionMethodExpression("#{stat_tap_in.handlechange1}")));
		        ajax.setUpdate("@form");
...
//			        
		         ...
		         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{stat_tap_in.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
				...
				lstDateYears.setId("lstDateYears");
				 ValueExpression valueExp = createValueExpression("#{stat_tap_in.date_year_deb}", Integer.class);
				...
				lstDateYearsFin.setId("lstDateYearsFin");
				 ValueExpression valueExp1 = createValueExpression("#{stat_tap_in.date_year_fin}", Integer.class);
			...
			ValueExpression valjour = createValueExpression("#{stat_tap_in.date_mois_debut}", Date.class);
			...
			ValueExpression valjourfin = createValueExpression("#{stat_tap_in.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			pie1=stat_tap_in_service.getStatTapOutByFilters(" t.typeSubscriber", "SUM(chargeTotal)", where_liste);
			if(this.getChoix_Voix()!="" && this.getChoix_Voix()!=null){
				where_liste.add(" typeSubscriber="+"'"+this.getChoix_Voix()+"'");
				SubTitle=SubTitle+"avec "+this.getChoix_Voix()+"comme Type Subscriber";
			}
			if(this.getChoix_typeCall()!="" && this.getChoix_typeCall()!=null){
				where_liste.add(" typeCall="+"'"+this.getChoix_typeCall()+"'");
				SubTitle=SubTitle+"avec "+this.getChoix_typeCall()+"comme Type Call";
			}
		...
		if(this.getChoix_periode().equals("Par An")){
	 			 staticListStatMsc = stat_tap_in_service.getStatTapOutByFilters("Extract (year from to_date("+type_date+",'YYMMDD'))", "SUM(nbAppel),SUM(chargeTotal),SUM(dureeTotal)", where_liste);
	 			
	 			
		 		 }else if(this.getChoix_periode().equals("Par Mois")){
		 			 staticListStatMsc = removeDuplicate(generateList, stat_tap_in_service.getStatTapOutByFilters("to_char(to_date("+type_date+",'YYMMDD'),'MM/YYYY')", "SUM(nbAppel),SUM(chargeTotal),SUM(dureeTotal)", where_liste));

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			
		 			 staticListStatMsc = removeDuplicate(generateList,stat_tap_in_service.getStatTapOutByFilters("to_char(to_date("+type_date+",'YYMMDD'),'YYYY-MM-DD')", "SUM(nbAppel),SUM(chargeTotal),SUM(dureeTotal)", where_liste));

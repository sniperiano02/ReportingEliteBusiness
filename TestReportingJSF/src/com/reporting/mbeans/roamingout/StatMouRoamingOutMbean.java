...
@ManagedBean(name="mou_roaout")
@ViewScoped
public class StatMouRoamingOutMbean {

	
	@EJB
	private MouRoamingOutRemote mou_service;
			 ...
			 chartDisplayed = false;
			 		...
			 		if(compJourOutputHeure!=null){
			 			...
			 			if(this.getChoix_periode().equals("Par Jour")){
			 				...
			 				ValueExpression valjour = createValueExpression("#{mou_roaout.date_ParJourDeb}", Date.class);
			 				...
			 				ValueExpression valjourfin = createValueExpression("#{mou_roaout.date_ParJourFin}", Date.class);
			 				...
			 				lstDateYears.setId("lstDateYears");
			 				 ValueExpression valueExp = createValueExpression("#{mou_roaout.date_year_deb}", Integer.class);
			 				...
			 				lstDateYearsFin.setId("lstDateYearsFin");
			 				 ValueExpression valueExp1 = createValueExpression("#{mou_roaout.date_year_fin}", Integer.class);
			 			...
			 			ValueExpression valjour = createValueExpression("#{mou_roaout.date_mois_debut}", Date.class);
			 			...
			 			ValueExpression valjourfin = createValueExpression("#{mou_roaout.date_mois_fin}", Date.class);
			 	...
			 	public void Valider(){
			 	 ...
			 	 if(this.getChoix_periode().equals("Par An")){
			 	 			 listMouRoa = mou_service.getStatMouOut("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(nbMinutes),SUM(nbRoa),(SUM(nbMinutes)/SUM(nbRoa))",  where_liste);
			 	 			
			 	 			
			 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
			 		 			 listMouRoa = removeDuplicate(generateList,mou_service.getStatMouOut("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(dureeTotal),SUM(parc),(SUM(dureeTotal)/SUM(parc))",   where_liste));

			 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
			 		 			 listMouRoa = removeDuplicate(generateList,mou_service.getStatMouOut("to_char(to_date(dateAppel,'YYMMDD'),'DD/MM/YYYY')", "SUM(nbMinutes),SUM(nbRoa),(SUM(nbMinutes)/SUM(nbRoa))",  where_liste));

			...
			AjaxBehavior ajax = new AjaxBehavior();
		       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
		        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
		            createActionMethodExpression("#{in_local.handlechange1}"),createActionMethodExpression("#{in_local.handlechange1}")));
		        ajax.setUpdate("@form");
...
//			        
		         ...
		         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{in_local.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
			...
			ValueExpression valjour = createValueExpression("#{in_local.date_mois_debut}", Date.class);
			...
			ValueExpression valjourfin = createValueExpression("#{in_local.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			pie1=statINLoc.getAllStatTrafficStatic("s.destination.typeDest", "SUM(duree)", "", "", where_liste);
			pie2=statINLoc.getAllStatTrafficStatic("s.plan.planTarifaire", "SUM(duree)", "", " ", where_liste);
		 if(this.getChoix_periode().equals("Par Heure")){
			staticListINLocal = removeDuplicate(generateList,statINLoc.getAllStatTrafficStatic("substring(trancheHoraire,1,2) ","SUM(moyenneDuree),SUM(duree),SUM(revenue),SUM(moyenneRevenue)" , "", "", where_liste));
				 }else if(this.getChoix_periode().equals("Par An")){
						staticListINLocal = removeDuplicate(generateList,statINLoc.getAllStatTrafficStatic("Extract (year from to_date(dateAppel,'YYMMDD')) ","SUM(moyenneDuree),SUM(duree),SUM(revenue),SUM(moyenneRevenue)" , "", "", where_liste));

		 		 }else if(this.getChoix_periode().equals("Par Mois")){
						staticListINLocal = removeDuplicate(generateList,statINLoc.getAllStatTrafficStatic("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY') ","SUM(moyenneDuree),SUM(duree),SUM(revenue),SUM(moyenneRevenue)" , "", "", where_liste));

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
						staticListINLocal = removeDuplicate(generateList,statINLoc.getAllStatTrafficStatic("to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD') ","SUM(moyenneDuree),SUM(duree),SUM(revenue),SUM(moyenneRevenue)" , "", "", where_liste));

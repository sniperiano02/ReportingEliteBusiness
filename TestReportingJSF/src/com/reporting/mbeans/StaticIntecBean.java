...
// 			}
		...
		if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 staticListStatIntec = stat_remote.getAllStatIntecStatic("substring(trancheHoraire,1,2)", "SUM(dureeMoyenne),SUM(dureeTotal),SUM(revenueMoyenne),SUM(montantTotal)", "", "", where_liste);
	 			 ...
	 			 staticListStatIntec = stat_remote.getAllStatIntecStatic("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(dureeMoyenne),SUM(dureeTotal),SUM(revenueMoyenne),SUM(montantTotal)", "", "", where_liste);
		 			 ...
		 			 staticListStatIntec = stat_remote.getAllStatIntecStatic("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(dureeMoyenne),SUM(dureeTotal),SUM(revenueMoyenne),SUM(montantTotal)", "", "", where_liste);

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			 staticListStatIntec = stat_remote.getAllStatIntecStatic("to_char(to_date(dateAppel,'YYMMDD'),'DD')", "SUM(dureeMoyenne),SUM(dureeTotal),SUM(revenueMoyenne),SUM(montantTotal)", "", "", where_liste);

...
@ManagedBean(name="arpu_prp")
@ViewScoped
public class ArpuMbean {
	
	@EJB
	private StatArpuRemote arpu_service;
			 ...
			 chartDisplayed = false;
			 		...
			 		if(compJourOutputHeure!=null){
			 			...
			 			if(this.getChoix_periode().equals("Par Jour")){
			 				...
			 				ValueExpression valjour = createValueExpression("#{arpu_prp.date_ParJourDeb}", Date.class);
			 				...
			 				ValueExpression valjourfin = createValueExpression("#{arpu_prp.date_ParJourFin}", Date.class);
			 				...
			 				lstDateYears.setId("lstDateYears");
			 				 ValueExpression valueExp = createValueExpression("#{arpu_prp.date_year_deb}", Integer.class);
			 				...
			 				lstDateYearsFin.setId("lstDateYearsFin");
			 				 ValueExpression valueExp1 = createValueExpression("#{arpu_prp.date_year_fin}", Integer.class);
			 			...
			 			ValueExpression valjour = createValueExpression("#{arpu_prp.date_mois_debut}", Date.class);
			 			...
			 			ValueExpression valjourfin = createValueExpression("#{arpu_prp.date_mois_fin}", Date.class);
			 	...
			 	public void Valider(){
			 	 ...
			 	 if(this.getChoix_periode().equals("Par An")){
			 	 			 listeArpu = arpu_service.getStatArpu("Extract (year from to_date(dateAppel,'YYMMDD'))", "AVG(revenueTotal),AVG(parcPrp),(AVG(revenueTotal)/AVG(parcPrp))",  "", where_liste);
			 	 			
			 	 			
			 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
			 		 			 listeArpu = removeDuplicate(generateList,arpu_service.getStatArpu("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "AVG(revenueTotal),AVG(parcPrp),(AVG(revenueTotal) / AVG(parcPrp))",  "", where_liste));

			 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
			 		 			 listeArpu = removeDuplicate(generateList,arpu_service.getStatArpu("to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD')", "AVG(revenueTotal),AVG(parcPrp),(AVG(revenueTotal) / AVG(parcPrp)) ", "", where_liste));

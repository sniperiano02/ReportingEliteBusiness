...
@ManagedBean(name="balance_secondaire")
@ViewScoped
public class BalanceSecondaireMbean {
	
	@EJB
	BalancePrincipeRemote balance_service;
	
	
	@EJB
	AccountINRemote account_in_service;

	private List<AccountIn> lst_account_in;
	public List<AccountIn> getLst_account_in() {
		return lst_account_in;
	}
	public void setLst_account_in(List<AccountIn> lst_account_in) {
		this.lst_account_in = lst_account_in;
	}
	private List<Object[]> listeBalancePrincipe;
	
	private AccountIn choix_account_in;
	
	public AccountIn getChoix_account_in() {
		return choix_account_in;
	}
	public void setChoix_account_in(AccountIn choix_account_in) {
		this.choix_account_in = choix_account_in;
	}
	...
	private List<Object[]> listeBalancePrincipe1;

public List<Object[]> getListeBalancePrincipe() {
	return listeBalancePrincipe;
}
public void setListeBalancePrincipe(List<Object[]> listeBalancePrincipe) {
	this.listeBalancePrincipe = listeBalancePrincipe;
}
public List<Object[]> getListeBalancePrincipe1() {
	return listeBalancePrincipe1;
}
public void setListeBalancePrincipe1(List<Object[]> listeBalancePrincipe1) {
	this.listeBalancePrincipe1 = listeBalancePrincipe1;
}
	...
	public void setDate_year(Integer date_year) {
		...
		private String choix_typeCall;
		 ...
		 @PostConstruct
		 public void init(){
			 liste_desAns = balance_service.getAllYears();
			 listeBalancePrincipe = new ArrayList<>();
			 listeBalancePrincipe1 = new ArrayList<>();
			 lst_account_in = account_in_service.getAllAccountsIn();
			 choix_account_in= new AccountIn();
				...
				chartDisplayed = false;
						...
						if(compJourOutputHeure!=null){
							...
							if(this.getChoix_periode().equals("Par Jour")){
								...
								ValueExpression valjour = createValueExpression("#{balance_secondaire.date_ParJourDeb}", Date.class);
							...
							addAjax(cld_jour_debut);
								...
								ValueExpression valjourfin = createValueExpression("#{balance_secondaire.date_ParJourFin}", Date.class);
							...
							ValueExpression valjour = createValueExpression("#{balance_secondaire.date_Parheure}", Date.class);
							...
							AjaxBehavior ajax = new AjaxBehavior();
						       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
						        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
						            createActionMethodExpression("#{balance_secondaire.handlechange1}"),createActionMethodExpression("#{balance_secondaire.handlechange1}")));
						        ajax.setUpdate("@form");
...
//							        
						         ...
						         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{balance_secondaire.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
								...
								lstDateYears.setId("lstDateYears");
								 ValueExpression valueExp = createValueExpression("#{balance_secondaire.date_year_deb}", Integer.class);
								...
								lstDateYearsFin.setId("lstDateYearsFin");
								 ValueExpression valueExp1 = createValueExpression("#{balance_secondaire.date_year_fin}", Integer.class);
							...
							ValueExpression valjour = createValueExpression("#{balance_secondaire.date_mois_debut}", Date.class);
							...
							ValueExpression valjourfin = createValueExpression("#{balance_secondaire.date_mois_fin}", Date.class);
			 	...
			 	public void Valider(){
			 		...
			 		if(this.getChoix_periode().equals(" ")){
						...
						if(this.getChoix_periode().equals("Par Jour")){
			 			...
			 			if( this.getChoix_account_in() != null ){
			 				
							where_liste.add("account.accountId ='"+this.getChoix_account_in().getAccountId()+"'");
							SubTitle = SubTitle+ " avec "+this.getChoix_account_in().getDescription()+" comme account IN";
			 	 ...
			 	 if(this.getChoix_periode().equals("Par An")){
			 	 			 ...
			 	 			 listeBalancePrincipe = balance_service.getBalance("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(conso),SUM(balanceInitial),SUM(balanceFinal),SUM(diff)",  "StatAccountIn", where_liste);
			 	 		
			 	 			 
			 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
						 	
			 		 			listeBalancePrincipe =balance_service.getBalance("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(conso),SUM(balanceInitial),SUM(balanceFinal),SUM(diff)",  "StatAccountIn", where_liste);
			 		 			

			 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
			 		 	
				 	 		
			 		 			listeBalancePrincipe =balance_service.getBalance("to_char(to_date(dateAppel,'YYMMDD'),'DD/MM/YYYY')", "SUM(conso),SUM(balanceInitial),SUM(balanceFinal),SUM(diff)", "StatAccountIn", where_liste);

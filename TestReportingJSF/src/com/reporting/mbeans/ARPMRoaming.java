					...
					AjaxBehavior ajax = new AjaxBehavior();
				       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
				        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
				            createActionMethodExpression("#{arpmRoaming.handlechange1}"),createActionMethodExpression("#{arpmRoaming.handlechange1}")));
				        ajax.setUpdate("@form");
...
//					        
				         ...
				         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{arpmRoaming.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
					...
					ValueExpression valjour = createValueExpression("#{arpmRoaming.date_mois_debut}", Date.class);
					...
					ValueExpression valjourfin = createValueExpression("#{arpmRoaming.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
		 ...
		 if(this.getChoix_periode().equals("Par Heure")){
			...
			// sc1.setMap(statINLoc.getMscBytranche("CAST(trancheHoraire AS integer)", "SUM(revenue)*60/SUM(duree)", "", "", where_liste));
			 resultarpmRoaming= removeDuplicate(generateList,statINLoc.getAllStatIN("substring(trancheHoraire,1,2)",where_liste));
	 		 }else if(this.getChoix_periode().equals("Par An")){
	 ...
	 resultarpmRoaming= statINLoc.getAllStatIN("Extract (year from to_date(dateAppel,'YYMMDD'))",where_liste);
			...
			// sc1.setMap(statINLoc.getMscByFilters("Extract (month from to_date(dateAppel,'YYMMDD'))", "SUM(revenue)*60/SUM(duree)", "", "Group By Extract (month from to_date(dateAppel,'YYMMDD')) ", where_liste));	
			 resultarpmRoaming= removeDuplicate(generateList,statINLoc.getAllStatIN("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')",where_liste));
		...
		// sc1.setMap(statINLoc.getMscByFilters("extract(day from  to_date(dateAppel,'YYMMDD'))", "SUM(revenue)*60/SUM(duree)", "", "Group By  extract(day from  to_date(dateAppel,'YYMMDD'))", where_liste));	
		 resultarpmRoaming= removeDuplicate(generateList,statINLoc.getAllStatIN("to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD')",where_liste));

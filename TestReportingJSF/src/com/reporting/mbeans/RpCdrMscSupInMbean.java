...
public void setSubTitle(String subTitle) {
	...
	public void loadDestinations(){
		...
		if(this.getChoix_periode().equals("Par Jour")){
			...
			where=" anomalie='duree MSC supérieur duree IN' AND to_date(dateAppel,'YYMMDD') = to_date("+"'"+selectedRecon[0].toString()+"'"+",'yyyy-MM-dd') AND type_destination.typeDest= '"+selectedTypeDestination[0].toString()+"'";
		 ...
		 where =" anomalie='duree MSC supérieur duree IN' AND Extract(year from to_date(dateAppel,'YYMMDD')) = "+Integer.valueOf(selectedRecon[0].toString())+"  And Extract(month from to_date(dateAppel,'YYMMDD')) >= 01 And Extract(month from to_date(dateAppel,'YYMMDD')) <= 12 AND type_destination.typeDest='"+selectedTypeDestination[0].toString()+"'";
 ...
 where="anomalie='duree MSC supérieur duree IN' AND to_date(dateAppel,'YYMMDD') BETWEEN to_date("+"'"+mois+"'"+",'DD/MM/YYYY') AND to_date("+"'"+mois1+"'"+",'DD/MM/YYYY')  AND type_destination.typeDest= '"+selectedTypeDestination[0].toString()+"'";  
...
//		System.out.println(deb);
	List<Object[]> result1 = new ArrayList<Object[]>();
		...
		if(this.getChoix_periode().equals("Par Jour")){
			...
			where=" anomalie='duree MSC supérieur duree IN' And codeMsc= '"+selectedPoint+"' AND to_date(dateAppel,'YYMMDD') = to_date("+"'"+selectedRecon[0].toString()+"'"+",'yyyy-MM-dd') ";
		 ...
		 where =" anomalie='duree MSC supérieur duree IN' And codeMsc='"+selectedPoint+"' AND Extract(year from to_date(dateAppel,'YYMMDD')) = "+Integer.valueOf(selectedRecon[0].toString())+"  And Extract(month from to_date(dateAppel,'YYMMDD')) >= 01 And Extract(month from to_date(dateAppel,'YYMMDD')) <= 12";
		xDetail="Extract(month from to_date(dateAppel,'YYMMDD')) ";
		yDetail = ",SUM(dureeTotalIn),SUM(kpiDuree)";
		...
		String mois = "01/"+selectedRecon[0].toString();
		String mois1 = "31/"+selectedRecon[0].toString();
 where="anomalie='duree MSC supérieur duree IN' AND to_date(dateAppel,'YYMMDD') BETWEEN to_date("+"'"+mois+"'"+",'DD/MM/YYYY') AND  to_date("+"'"+mois1+"'"+",'DD/MM/YYYY') AND codeMsc='"+selectedPoint+"'";
xDetail="to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD') ";
yDetail = ",SUM(dureeTotalIn),SUM(kpiDuree)";
//resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconMscIn(xDetail, where);
			...
			AjaxBehavior ajax = new AjaxBehavior();
		       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
		        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
		            createActionMethodExpression("#{msc_sup_in.handlechange1}"),createActionMethodExpression("#{msc_sup_in.handlechange1}")));
		        ajax.setUpdate("@form");
...
//			        
		         ...
		         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{msc_sup_in.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
			...
			ValueExpression valjour = createValueExpression("#{msc_sup_in.date_mois_debut}", Date.class);
			...
			ValueExpression valjourfin = createValueExpression("#{msc_sup_in.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				String deb = df.format(this.getDate_mois_debut());
				String fin = df.format(this.getDate_mois_fin());
				String where=" anomalie='duree MSC supérieur duree IN' AND  to_date(dateAppel,'YYMMDD') Between to_date("+"'"+deb+"'"+",'MM-YYYY') And to_date("+"'"+fin+"'"+",'MM-YYYY')";
...
//		System.out.println(deb);
	List<Object[]> result1 = new ArrayList<Object[]>();
		...
		String mois = "01/"+selectedRecon[0].toString();
		String mois1 = "31/"+selectedRecon[0].toString();
 where="anomalie='duree MSC supérieur duree IN' AND to_date(dateAppel,'YYMMDD') BETWEEN to_date("+"'"+mois+"'"+",'DD/MM/YYYY') AND to_date("+"'"+mois1+"'"+",'DD/MM/YYYY') ";
xDetail="to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD')  ";
yDetail = ",SUM(dureeTotalIn),SUM(kpiDuree)";
resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconMscIn(xDetail, where);
Subtitle1="Par Jour en "+selectedRecon[0].toString();

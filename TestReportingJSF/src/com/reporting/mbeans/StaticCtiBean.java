...
@ManagedBean
@ViewScoped
public class StaticCtiBean implements Serializable {
	...
	private String choix_type;
	 ...
	 private List<Object[]> staticListStatCti = new ArrayList<>();
	 
	 
	 public List<Object[]> getStaticListStatCti() {
		return staticListStatCti;
	}
	 public void setStaticListStatCti(List<Object[]> staticListStatCti) {
		this.staticListStatCti = staticListStatCti;
	}
...
chartDisplayed = false;
		...
		if(compJourOutputHeure!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				ValueExpression valjour = createValueExpression("#{staticCtiBean.date_ParJourDeb}", Date.class);
				...
				ValueExpression valjourfin = createValueExpression("#{staticCtiBean.date_ParJourFin}", Date.class);
			...
			ValueExpression valjour = createValueExpression("#{staticCtiBean.date_Parheure}", Date.class);
			...
			AjaxBehavior ajax = new AjaxBehavior();
		       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
		        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
		            createActionMethodExpression("#{staticCtiBean.handlechange1}"),createActionMethodExpression("#{staticCtiBean.handlechange1}")));
		        ajax.setUpdate("@form");
...
//			        
		         ...
		         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{staticCtiBean.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
				...
				lstDateYears.setId("lstDateYears");
				 ValueExpression valueExp = createValueExpression("#{staticCtiBean.date_year_deb}", Integer.class);
				...
				lstDateYearsFin.setId("lstDateYearsFin");
				 ValueExpression valueExp1 = createValueExpression("#{staticCtiBean.date_year_fin}", Integer.class);
			...
			ValueExpression valjour = createValueExpression("#{staticCtiBean.date_mois_debut}", Date.class);
			...
			ValueExpression valjourfin = createValueExpression("#{staticCtiBean.date_mois_fin}", Date.class);
...
// 			}
		...
		pie1=stat_remote.getCtiByFilters1("operateur.codeOperateurs", "dureeTotal)","StatCtiInter", "SUM(", "Group By operateur.codeOperateurs ", where_liste);
		pie2=stat_remote.getCtiByFilters1("typeStat", "dureeTotal)","StatCtiInter", "SUM(", "Group By typeStat ", where_liste);
		...
		if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 staticListStatCti = stat_remote.getAllStatCtiStatic("substring(trancheHoraire,1,2)", "SUM(dureeMoyenne),SUM(dureeTotal),SUM(nbMoyenne),SUM(nombreTotal)", "", "", where_liste);
		
	 		 }else if(this.getChoix_periode().equals("Par An")){
	 			 staticListStatCti = stat_remote.getAllStatCtiStatic("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(dureeMoyenne),SUM(dureeTotal),SUM(nbMoyenne),SUM(nombreTotal)", "", "", where_liste);

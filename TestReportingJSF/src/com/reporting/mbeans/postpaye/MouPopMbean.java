...
import com.reporting.metier.interfaces.StatMouPopRemote;


@ManagedBean(name="mou_pop")
@ViewScoped
public class MouPopMbean {

	
		
		@EJB
		private StatMouPopRemote mou_service;
				 ...
				 chartDisplayed = false;
				 		...
				 		if(compJourOutputHeure!=null){
				 			...
				 			if(this.getChoix_periode().equals("Par Jour")){
				 				...
				 				ValueExpression valjour = createValueExpression("#{mou_pop.date_ParJourDeb}", Date.class);
				 				...
				 				ValueExpression valjourfin = createValueExpression("#{mou_pop.date_ParJourFin}", Date.class);
				 				...
				 				lstDateYears.setId("lstDateYears");
				 				 ValueExpression valueExp = createValueExpression("#{mou_pop.date_year_deb}", Integer.class);
				 				...
				 				lstDateYearsFin.setId("lstDateYearsFin");
				 				 ValueExpression valueExp1 = createValueExpression("#{mou_pop.date_year_fin}", Integer.class);
				 			...
				 			ValueExpression valjour = createValueExpression("#{mou_pop.date_mois_debut}", Date.class);
				 			...
				 			ValueExpression valjourfin = createValueExpression("#{mou_pop.date_mois_fin}", Date.class);
				 	...
				 	public void Valider(){
				 	 ...
				 	 if(this.getChoix_periode().equals("Par An")){
				 	 			 listeArpu = mou_service.getStatMouPop("Extract (year from to_date(dateAppel,'YYMMDD'))", "AVG(nbMinutes),AVG(parcPop),(AVG(nbMinutes)/AVG(parcPop))",  "", where_liste);
				 	 			
				 	 			
				 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
				 		 			 listeArpu = mou_service.getStatMouPop("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "AVG(nbMinutes),AVG(parcPop),(AVG(nbMinutes) / AVG(parcPop))",  "", where_liste);

				 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
				 		 			 listeArpu = mou_service.getStatMouPop("to_char(to_date(dateAppel,'YYMMDD'),'DD/MM/YYYY')", "AVG(nbMinutes),AVG(parcPop),(AVG(nbMinutes) / AVG(parcPop)) ", "", where_liste);

...
@ManagedBean()
@ViewScoped
public class StaticMscPostBean implements Serializable {
...
chartDisplayed = false;
		...
		if(compJourOutputHeure!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				ValueExpression valjour = createValueExpression("#{staticMscPostBean.date_ParJourDeb}", Date.class);
				...
				ValueExpression valjourfin = createValueExpression("#{staticMscPostBean.date_ParJourFin}", Date.class);
			...
			ValueExpression valjour = createValueExpression("#{staticMscPostBean.date_Parheure}", Date.class);
			...
			AjaxBehavior ajax = new AjaxBehavior();
		       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
		        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
		            createActionMethodExpression("#{staticMscPostBean.handlechange1}"),createActionMethodExpression("#{staticMscPostBean.handlechange1}")));
		        ajax.setUpdate("@form");
...
//			        
		         ...
		         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{staticMscPostBean.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
				...
				lstDateYears.setId("lstDateYears");
				 ValueExpression valueExp = createValueExpression("#{staticMscPostBean.date_year_deb}", Integer.class);
				...
				lstDateYearsFin.setId("lstDateYearsFin");
				 ValueExpression valueExp1 = createValueExpression("#{staticMscPostBean.date_year_fin}", Integer.class);
			...
			ValueExpression valjour = createValueExpression("#{staticMscPostBean.date_mois_debut}", Date.class);
			...
			ValueExpression valjourfin = createValueExpression("#{staticMscPostBean.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			where_liste.add(" typeSubscriber='pop'");
		 ...
		 if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 staticListStatMsc = stat_remote.getAllStatMscStatic("substring(trancheHoraire,1,2)", "SUM(moyenneDuree),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);
	 			 ...
	 			 staticListStatMsc = stat_remote.getAllStatMscStatic("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(moyenneDuree),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);
		 			 ...
		 			 staticListStatMsc = stat_remote.getAllStatMscStatic("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(moyenneDuree),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			 staticListStatMsc = stat_remote.getAllStatMscStatic("to_char(to_date(dateAppel,'YYMMDD'),'DD')", "SUM(moyenneDuree),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);

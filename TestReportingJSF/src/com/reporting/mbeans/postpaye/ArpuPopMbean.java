...
import com.reporting.metier.interfaces.StatArpuPopRemote;
import com.reporting.metier.interfaces.StatArpuRemote;
@ManagedBean(name="arpu_pop")
@ViewScoped
public class ArpuPopMbean {
	
	
		@EJB
		private StatArpuPopRemote arpu_service;
		...
		public void setDate_year(Integer date_year) {
			...
			private String choix_typeCall;
			 ...
			 @PostConstruct
			 public void init(){
				 liste_desAns = arpu_service.getAllYears();
				 ...
				 chartDisplayed = false;
				 		...
				 		if(compJourOutputHeure!=null){
				 			...
				 			if(this.getChoix_periode().equals("Par Jour")){
				 				...
				 				ValueExpression valjour = createValueExpression("#{arpu_pop.date_ParJourDeb}", Date.class);
				 				...
				 				ValueExpression valjourfin = createValueExpression("#{arpu_pop.date_ParJourFin}", Date.class);
				 				...
				 				lstDateYears.setId("lstDateYears");
				 				 ValueExpression valueExp = createValueExpression("#{arpu_pop.date_year_deb}", Integer.class);
				 				...
				 				lstDateYearsFin.setId("lstDateYearsFin");
				 				 ValueExpression valueExp1 = createValueExpression("#{arpu_pop.date_year_fin}", Integer.class);
				 			...
				 			ValueExpression valjour = createValueExpression("#{arpu_pop.date_mois_debut}", Date.class);
				 			...
				 			ValueExpression valjourfin = createValueExpression("#{arpu_pop.date_mois_fin}", Date.class);
				 	...
				 	public void Valider(){
				 	 ...
				 	 if(this.getChoix_periode().equals("Par An")){
				 	 			 listeArpu = arpu_service.getStatArpuPop("Extract (year from to_date(dateAppel,'YYMMDD'))", "AVG(revenueTotal),AVG(parcPop),(AVG(revenueTotal)/AVG(parcPop))",  "", where_liste);
				 	 			
				 	 			
				 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
				 		 			 listeArpu = arpu_service.getStatArpuPop("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "AVG(revenueTotal),AVG(parcPop),(AVG(revenueTotal) / AVG(parcPop))",  "", where_liste);

				 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
				 		 			 listeArpu = arpu_service.getStatArpuPop("to_char(to_date(dateAppel,'YYMMDD'),'DD/MM/YYYY')", "AVG(revenueTotal),AVG(parcPop),(AVG(revenueTotal) / AVG(parcPop)) ", "", where_liste);

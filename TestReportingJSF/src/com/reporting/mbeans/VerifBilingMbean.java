...
@ManagedBean(name="verif_Biling")
@ViewScoped
public class VerifBilingMbean {
	
	@EJB
	VerifBilingRemote verif_biling_service;
			 ...
			 chartDisplayed = false;
			 		...
			 		if(compJourOutputHeure!=null){
			 			...
			 			if(this.getChoix_periode().equals("Par Jour")){
			 				...
			 				ValueExpression valjour = createValueExpression("#{verif_Biling.date_ParJourDeb}", Date.class);
			 				...
			 				ValueExpression valjourfin = createValueExpression("#{verif_Biling.date_ParJourFin}", Date.class);
			 				...
			 				lstDateYears.setId("lstDateYears");
			 				 ValueExpression valueExp = createValueExpression("#{verif_Biling.date_year_deb}", Integer.class);
			 				...
			 				lstDateYearsFin.setId("lstDateYearsFin");
			 				 ValueExpression valueExp1 = createValueExpression("#{verif_Biling.date_year_fin}", Integer.class);
			 			...
			 			ValueExpression valjour = createValueExpression("#{verif_Biling.date_mois_debut}", Date.class);
			 			...
			 			ValueExpression valjourfin = createValueExpression("#{verif_Biling.date_mois_fin}", Date.class);
			 	...
			 	public void Valider(){
			 	 ...
			 	 if(this.getChoix_periode().equals("Par An")){
			 	 			 listeVerifBiling = verif_biling_service.getStatVerifBiling("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(nbAppelCti),SUM(dureeCti),SUM(montantCti),SUM(nbAppelIntec),SUM(dureeIntec),SUM(montantIntec)",  "", where_liste);
			 	 			
			 	 			
			 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
			 		 			 listeVerifBiling = removeDuplicate(generateList,verif_biling_service.getStatVerifBiling("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(nbAppelCti),SUM(dureeCti),SUM(montantCti),SUM(nbAppelIntec),SUM(dureeIntec),SUM(montantIntec)",  "", where_liste));

			 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
			 		 			 listeVerifBiling = removeDuplicate(generateList,verif_biling_service.getStatVerifBiling("to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD')", "SUM(nbAppelCti),SUM(dureeCti),SUM(montantCti),SUM(nbAppelIntec),SUM(dureeIntec),SUM(montantIntec)", "", where_liste));
			 	...
			 	public void getDetail(){
					...
					listeDetailsVerifBiling=verif_biling_service.getDetailsStatVerifBiling(where);

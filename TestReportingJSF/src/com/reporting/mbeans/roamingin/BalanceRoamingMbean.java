...
@ManagedBean(name="balance_roadashboard")
@ViewScoped
public class BalanceRoamingMbean {
			 ...
			 chartDisplayed = false;
			 		...
			 		if(compJourOutputHeure!=null){
			 			...
			 			if(this.getChoix_periode().equals("Par Jour")){
			 				...
			 				ValueExpression valjour = createValueExpression("#{balance_roadashboard.date_ParJourDeb}", Date.class);
			 				...
			 				ValueExpression valjourfin = createValueExpression("#{balance_roadashboard.date_ParJourFin}", Date.class);
			 				...
			 				lstDateYears.setId("lstDateYears");
			 				 ValueExpression valueExp = createValueExpression("#{balance_roadashboard.date_year_deb}", Integer.class);
			 				...
			 				lstDateYearsFin.setId("lstDateYearsFin");
			 				 ValueExpression valueExp1 = createValueExpression("#{balance_roadashboard.date_year_fin}", Integer.class);
			 			...
			 			ValueExpression valjour = createValueExpression("#{balance_roadashboard.date_mois_debut}", Date.class);
			 			...
			 			ValueExpression valjourfin = createValueExpression("#{balance_roadashboard.date_mois_fin}", Date.class);
			 	...
			 	public void Valider(){
			 		...
			 		if(this.getChoix_periode().equals(" ")){
				        ...
				        for (DateTime d : between) {
		 	 			...
		 	 			listOperBalance = balance_service.getStatBalance("operateur.operateur", "SUM(produit),SUM(produitConverti),SUM(charge),SUM(chargeConverti),SUM(revenueConverti)-SUM(chargeConverti),SUM(revenue)-SUM(charge)", where_liste);

			 			
			 	
			 				
			 	 if(this.getChoix_periode().equals("Par An")){
			 	 			 listMouRoa = balance_service.getStatBalance("Extract (year from to_date("+choix_typedate+",'YYMMDD'))", "SUM(charge),SUM(produit)",  where_liste);
			 	 			
			 	 			
			 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
			 		 			 listMouRoa = removeDuplicate(generateList,balance_service.getStatBalance("to_char(to_date("+choix_typedate+",'YYMMDD'),'MM/YYYY')", "SUM(charge),SUM(produit)",   where_liste));

			 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
			 		 			 listMouRoa = removeDuplicate(generateList,balance_service.getStatBalance("to_char(to_date("+choix_typedate+",'YYMMDD'),'DD/MM/YYYY')", "SUM(charge),SUM(produit)",  where_liste));

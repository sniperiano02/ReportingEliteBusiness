...
@ManagedBean(name="stat_tap_out")
@ViewScoped
public class StatTapOutMbean {

	
	
	@EJB
	private StatTapOutRemote stat_tap_out_service;
...
public void init(){
		...
		liste_desAns=stat_tap_out_service.getAllYears();
...
chartDisplayed = false;
		...
		if(compJourOutputHeure!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				ValueExpression valjour = createValueExpression("#{stat_tap_out.date_ParJourDeb}", Date.class);
			...
			addAjax(cld_jour_debut);
				...
				ValueExpression valjourfin = createValueExpression("#{stat_tap_out.date_ParJourFin}", Date.class);
			...
			ValueExpression valjour = createValueExpression("#{stat_tap_out.date_Parheure}", Date.class);
			...
			AjaxBehavior ajax = new AjaxBehavior();
		       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
		        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
		            createActionMethodExpression("#{stat_tap_out.handlechange1}"),createActionMethodExpression("#{stat_tap_out.handlechange1}")));
		        ajax.setUpdate("@form");
...
//			        
		         ...
		         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{stat_tap_out.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
				...
				lstDateYears.setId("lstDateYears");
				 ValueExpression valueExp = createValueExpression("#{stat_tap_out.date_year_deb}", Integer.class);
				...
				lstDateYearsFin.setId("lstDateYearsFin");
				 ValueExpression valueExp1 = createValueExpression("#{stat_tap_out.date_year_fin}", Integer.class);
			...
			ValueExpression valjour = createValueExpression("#{stat_tap_out.date_mois_debut}", Date.class);
			...
			ValueExpression valjourfin = createValueExpression("#{stat_tap_out.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			pie1=stat_tap_out_service.getStatTapOutByFilters("CASE  WHEN t.typeCall='MO' THEN 'Voix' ELSE 'other' END", "SUM(revenue)", where_liste);
		...
		if(this.getChoix_periode().equals("Par An")){
	 			 staticListStatMsc = stat_tap_out_service.getStatTapOutByFilters("Extract (year from to_date("+type_date+",'YYMMDD'))", "SUM(nbAppel),SUM(duree),SUM(revenue)", where_liste);
	 			
	 			
		 		 }else if(this.getChoix_periode().equals("Par Mois")){
		 			 staticListStatMsc = removeDuplicate(generateList, stat_tap_out_service.getStatTapOutByFilters("to_char(to_date("+type_date+",'YYMMDD'),'MM/YYYY')", "SUM(nbAppel),SUM(duree),SUM(revenue)", where_liste));

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			
		 			 staticListStatMsc = removeDuplicate(generateList,stat_tap_out_service.getStatTapOutByFilters("to_char(to_date("+type_date+",'YYMMDD'),'YYYY-MM-DD')", "SUM(nbAppel),SUM(duree),SUM(revenue)", where_liste));

...
import com.reporting.mbeans.HighChart;
import com.reporting.mbeans.SerieChart;
import com.reporting.metier.interfaces.StatReconTapoutRemote;



@ManagedBean(name="recon_msc_tapout")
@ViewScoped
public class StatReconTapoutMbean {

	
	@EJB
	private StatReconTapoutRemote stat_tapout_service;
...
public void setSubTitle(String subTitle) {
	...
	public void loadDestinations(){
		...
		if(this.getChoix_periode().equals("Par Jour")){
			...
			where="  to_date(dateAppel,'YYMMDD') = to_date("+"'"+selectedRecon[0].toString()+"'"+",'yyyy-MM-dd') AND operateur.operateur= '"+selectedTypeDestination[0].toString()+"'";
		
			

			
			listbyDest = stat_tapout_service.getDetailsOperateurStatReconTapout("destination.dest", where);
		 ...
		 where ="  Extract(year from to_date(dateAppel,'YYMMDD')) = "+Integer.valueOf(selectedRecon[0].toString())+"  And Extract(month from to_date(dateAppel,'YYMMDD')) >= 01 And Extract(month from to_date(dateAppel,'YYMMDD')) <= 12 AND operateur.operateur='"+selectedTypeDestination[0].toString()+"'";
 ...
 where=" to_date(dateAppel,'YYMMDD') BETWEEN to_date("+"'"+mois+"'"+",'DD/MM/YYYY') AND to_date("+"'"+mois1+"'"+",'DD/MM/YYYY')  AND operateur.operateur= '"+selectedTypeDestination[0].toString()+"'";  
...
listbyDest = stat_tapout_service.getDetailsOperateurStatReconTapout("destination.dest", where);
	...
	private List<Object[]> DetailsMSC = new ArrayList<>();
	
	public List<Object[]> getDetailsMSC() {
		return DetailsMSC;
	}
	public void setDetailsMSC(List<Object[]> detailsMSC) {
		DetailsMSC = detailsMSC;
	}
	private List<Object[]> lst_detailsLine = new ArrayList<>();
	
	public List<Object[]> getLst_detailsLine() {
		return lst_detailsLine;
	}
	public void setLst_detailsLine(List<Object[]> lst_detailsLine) {
		this.lst_detailsLine = lst_detailsLine;
	}
	...
	public void init(){
			...
			DetailsMSC = new ArrayList<>();
		    lst_detailsLine = new ArrayList<>();
...
//		System.out.println(deb);
	List<Object[]> result1 = new ArrayList<Object[]>();
		...
		if(this.getChoix_periode().equals("Par Jour")){
			...
			where=" msc= '"+selectedPoint+"' AND to_date(dateAppel,'YYMMDD') = to_date("+"'"+selectedRecon[0].toString()+"'"+",'yyyy-MM-dd') ";
		
			
			xDetail="CAST(trancheHoraire AS integer)  ";
			yDetail = "coalesce(SUM(dureeTotalMsc),0),coalesce(SUM(kpiDuree),0)";
		 ...
		 where =" msc='"+selectedPoint+"' AND Extract(year from to_date(dateAppel,'YYMMDD')) = "+Integer.valueOf(selectedRecon[0].toString())+"  And Extract(month from to_date(dateAppel,'YYMMDD')) >= 01 And Extract(month from to_date(dateAppel,'YYMMDD')) <= 12";
		...
		String mois = "01/"+selectedRecon[0].toString();
		String mois1 = "31/"+selectedRecon[0].toString();
 where=" to_date(dateAppel,'YYMMDD') BETWEEN to_date("+"'"+mois+"'"+",'DD/MM/YYYY') AND  to_date("+"'"+mois1+"'"+",'DD/MM/YYYY') AND msc='"+selectedPoint+"'";
xDetail="to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD') ";
yDetail = "coalesce(SUM(dureeTotalMsc),0),coalesce(SUM(kpiDuree),0)";
//resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconMscIn(xDetail, where);
Subtitle1="Par Jour en "+selectedRecon[0].toString();
		...
		lst_detailsLine = stat_tapout_service.getDetailsStatReconTapout(xDetail,yDetail, where);
...
public void handlechange(AjaxBehaviorEvent event){
		...
		if(compJourOutputHeure!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				ValueExpression valjour = createValueExpression("#{recon_msc_tapout.date_ParJourDeb}", Date.class);
				...
				ValueExpression valjourfin = createValueExpression("#{recon_msc_tapout.date_ParJourFin}", Date.class);
			...
			ValueExpression valjour = createValueExpression("#{recon_msc_tapout.date_Parheure}", Date.class);
			...
			AjaxBehavior ajax = new AjaxBehavior();
		       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
		        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
		            createActionMethodExpression("#{recon_msc_tapout.handlechange1}"),createActionMethodExpression("#{recon_msc_tapout.handlechange1}")));
		        ajax.setUpdate("@form");
...
//			        
		         ...
		         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{recon_msc_tapout.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
				...
				lstDateYears.setId("lstDateYears");
				 ValueExpression valueExp = createValueExpression("#{recon_msc_tapout.date_year_deb}", Integer.class);
				...
				lstDateYearsFin.setId("lstDateYearsFin");
				 ValueExpression valueExp1 = createValueExpression("#{recon_msc_tapout.date_year_fin}", Integer.class);
			...
			ValueExpression valjour = createValueExpression("#{recon_msc_tapout.date_mois_debut}", Date.class);
			...
			ValueExpression valjourfin = createValueExpression("#{recon_msc_tapout.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			if(!this.getChoix_voix().equals("") && this.getChoix_voix()!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
					...
					resultCDRInexistant= removeDuplicate(generateList, stat_tapout_service.getAllStatReconTapout("to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD')","SUM(nbTotalMsc),SUM(dureeTotalMsc),SUM(nbTotalTap),SUM(dureeTotalTap),SUM(nbAnomalie),SUM(dureeAnomalie),SUM(impact)",where_liste));
				...
				resultCDRInexistant= stat_tapout_service.getAllStatReconTapout("Extract (year from to_date(dateAppel,'YYMMDD'))","SUM(nbTotalMsc),SUM(dureeTotalMsc),SUM(nbTotalTap),SUM(dureeTotalTap),SUM(nbAnomalie),SUM(dureeAnomalie),SUM(impact)",where_liste);
		...
		resultCDRInexistant= removeDuplicate(generateList, stat_tapout_service.getAllStatReconTapout("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY') ","SUM(nbTotalMsc),SUM(dureeTotalMsc),SUM(nbTotalTap),SUM(dureeTotalTap),SUM(nbAnomalie),SUM(dureeAnomalie),SUM(impact)",where_liste));
...
//		System.out.println(deb);
	List<Object[]> result1 = new ArrayList<Object[]>();
		...
		if(this.getChoix_periode().equals("Par Jour")){
			...
			where="  to_date(dateAppel,'YYMMDD') = to_date("+"'"+selectedRecon[0].toString()+"'"+",'yyyy-MM-dd') and typeCall = '"+this.getChoix_voix()+"'";
		
			
			xDetail="operateur.operateur  ";
			
			resultCDRDetailsInexistant = stat_tapout_service.getDetailsOperateurStatReconTapout(xDetail, where);
			Subtitle1="Par tranche Horaire"+selectedRecon[0].toString();
		 ...
		 where ="  Extract(year from to_date(dateAppel,'YYMMDD')) = "+Integer.valueOf(selectedRecon[0].toString())+"  And Extract(month from to_date(dateAppel,'YYMMDD')) >= 01 And Extract(month from to_date(dateAppel,'YYMMDD')) <= 12 and typeCall = '"+this.getChoix_voix()+"'";
		xDetail="operateur.operateur ";
		...
		String mois = "01/"+selectedRecon[0].toString();
		String mois1 = "31/"+selectedRecon[0].toString();
 where=" to_date(dateAppel,'YYMMDD') BETWEEN to_date("+"'"+mois+"'"+",'DD/MM/YYYY') AND to_date("+"'"+mois1+"'"+",'DD/MM/YYYY') and typeCall = '"+this.getChoix_voix()+"'";
xDetail="operateur.operateur ";
...
Subtitle1="Par Jour en "+selectedRecon[0].toString();
		...
		DetailsMSC = stat_tapout_service.getDetailsStatReconTapout("COALESCE(msc, 'MSC non définis' )","SUM(dureeAnomalie)", where);

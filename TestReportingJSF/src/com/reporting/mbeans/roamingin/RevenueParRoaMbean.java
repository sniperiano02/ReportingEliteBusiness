...
@ManagedBean(name="rev_roa")
@ViewScoped
public class RevenueParRoaMbean {


	@EJB
	private MouRoamingRemote mou_service;
			 ...
			 chartDisplayed = false;
			 		...
			 		if(compJourOutputHeure!=null){
			 			...
			 			if(this.getChoix_periode().equals("Par Jour")){
			 				...
			 				ValueExpression valjour = createValueExpression("#{nb_roa.date_ParJourDeb}", Date.class);
			 				...
			 				ValueExpression valjourfin = createValueExpression("#{nb_roa.date_ParJourFin}", Date.class);
			 				...
			 				lstDateYears.setId("lstDateYears");
			 				 ValueExpression valueExp = createValueExpression("#{nb_roa.date_year_deb}", Integer.class);
			 				...
			 				lstDateYearsFin.setId("lstDateYearsFin");
			 				 ValueExpression valueExp1 = createValueExpression("#{nb_roa.date_year_fin}", Integer.class);
			 			...
			 			ValueExpression valjour = createValueExpression("#{nb_roa.date_mois_debut}", Date.class);
			 			...
			 			ValueExpression valjourfin = createValueExpression("#{nb_roa.date_mois_fin}", Date.class);
			 		...
			 		if(this.getChoix_periode().equals(" ")){
				        ...
				        for (DateTime d : between) {
			 			...
			 			if(this.getChoix_operateur()!=null){
							...
							SubTitle=SubTitle+"avec "+this.getChoix_operateur().getOperateur()+"comme Operateur";
						}

			 	
			 				
			 	 if(this.getChoix_periode().equals("Par An")){
			 	 			 listMouRoa = mou_service.getStatMou("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(nbMinutes),SUM(nbRoa),SUM(revenue),SUM(revenueConverti),coalesce((SUM(revenue)/SUM(nbRoa),0) ",  where_liste);
			 	 			
			 	 			
			 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
			 		 			 listMouRoa = removeDuplicate(generateList,mou_service.getStatMou("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(nbMinutes),SUM(nbRoa),SUM(revenue),SUM(revenueConverti),coalesce((SUM(revenue)/SUM(nbRoa),0) ",   where_liste));

			 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
			 		 			 listMouRoa = removeDuplicate(generateList,mou_service.getStatMou("to_char(to_date(dateAppel,'YYMMDD'),'DD/MM/YYYY')", "SUM(nbMinutes),SUM(nbRoa),SUM(revenue),SUM(revenueConverti),coalesce((SUM(revenue)/SUM(nbRoa),0) ",  where_liste));

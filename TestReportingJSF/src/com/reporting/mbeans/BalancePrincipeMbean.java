...
@ManagedBean(name="balance_principe")
@ViewScoped
public class BalancePrincipeMbean {
				...
				chartDisplayed = false;
						...
						if(compJourOutputHeure!=null){
							...
							if(this.getChoix_periode().equals("Par Jour")){
								...
								ValueExpression valjour = createValueExpression("#{balance_principe.date_ParJourDeb}", Date.class);
							...
							addAjax(cld_jour_debut);
								...
								ValueExpression valjourfin = createValueExpression("#{balance_principe.date_ParJourFin}", Date.class);
							...
							ValueExpression valjour = createValueExpression("#{balance_principe.date_Parheure}", Date.class);
							...
							AjaxBehavior ajax = new AjaxBehavior();
						       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
						        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
						            createActionMethodExpression("#{balance_principe.handlechange1}"),createActionMethodExpression("#{balance_principe.handlechange1}")));
						        ajax.setUpdate("@form");
...
//							        
						         ...
						         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{balance_principe.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
								...
								lstDateYears.setId("lstDateYears");
								 ValueExpression valueExp = createValueExpression("#{balance_principe.date_year_deb}", Integer.class);
								...
								lstDateYearsFin.setId("lstDateYearsFin");
								 ValueExpression valueExp1 = createValueExpression("#{balance_principe.date_year_fin}", Integer.class);
							...
							ValueExpression valjour = createValueExpression("#{balance_principe.date_mois_debut}", Date.class);
							...
							ValueExpression valjourfin = createValueExpression("#{balance_principe.date_mois_fin}", Date.class);
			 	...
			 	public void Valider(){
			 	 ...
			 	 if(this.getChoix_periode().equals("Par An")){
			 	 			 ...
			 	 			 listeBalancePrincipe = balance_service.getBalance("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(consoRevenue),SUM(balanceRevenue),SUM(diff)",  "StatRevenueIn", where_liste);
			 	 		
			 	 			 
			 		 		 }else if(this.getChoix_periode().equals("Par Mois")){
						 	
			 		 			listeBalancePrincipe =balance_service.getBalance("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(consoRevenue),SUM(balanceRevenue),SUM(diff)",  "StatRevenueIn", where_liste);
			 		 			

			 		 		 }else if (this.getChoix_periode().equals("Par Jour")){
			 		 	
				 	 		
			 		 			listeBalancePrincipe = balance_service.getBalance("to_char(to_date(dateAppel,'YYMMDD'),'DD/MM/YYYY')", "SUM(consoRevenue),SUM(balanceRevenue),SUM(diff)", "StatRevenueIn", where_liste);

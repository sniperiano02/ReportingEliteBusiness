...
public class PMPM {
	...
	private List<Object[]> resultatpmpm;
	...
	public List<Object[]> getResultatpmpm() {
		return resultatpmpm;
	}
	public void setResultatpmpm(List<Object[]> resultatpmpm) {
		this.resultatpmpm = resultatpmpm;
	}
		...
		chartDisplayed = false;
				...
				if(compJourOutputHeure!=null){
					...
					if(this.getChoix_periode().equals("Par Jour")){
						...
						ValueExpression valjour = createValueExpression("#{pmpm.date_ParJourDeb}", Date.class);
						...
						ValueExpression valjourfin = createValueExpression("#{pmpm.date_ParJourFin}", Date.class);
					...
					ValueExpression valjour = createValueExpression("#{pmpm.date_Parheure}", Date.class);
					...
					AjaxBehavior ajax = new AjaxBehavior();
				       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
				        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
				            createActionMethodExpression("#{pmpm.handlechange1}"),createActionMethodExpression("#{pmpm.handlechange1}")));
				        ajax.setUpdate("@form");
...
//					        
				         ...
				         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{pmpm.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
						...
						lstDateYears.setId("lstDateYears");
						 ValueExpression valueExp = createValueExpression("#{pmpm.date_year_deb}", Integer.class);
						...
						lstDateYearsFin.setId("lstDateYearsFin");
						 ValueExpression valueExp1 = createValueExpression("#{pmpm.date_year_fin}", Integer.class);
					...
					ValueExpression valjour = createValueExpression("#{pmpm.date_mois_debut}", Date.class);
					...
					ValueExpression valjourfin = createValueExpression("#{pmpm.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 resultatpmpm = interco_service.getStatInterco("substring(trancheHoraire,1,2)", "SUM(montantConverti)/(SUM(dureeTotal)/60)", "", where_liste);
		
	 		 }else if(this.getChoix_periode().equals("Par An")){
	 			 resultatpmpm = interco_service.getStatInterco("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(montantConverti)/(SUM(dureeTotal)/60)", "", where_liste);
	 			
	 			
		 		 }else if(this.getChoix_periode().equals("Par Mois")){
		 			 resultatpmpm = interco_service.getStatInterco("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(montantConverti)/(SUM(dureeTotal)/60)", "",  where_liste);

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			 resultatpmpm = interco_service.getStatInterco("to_char(to_date(dateAppel,'YYMMDD'),'DD')", "SUM(montantConverti)/(SUM(dureeTotal)/60)", "",  where_liste);

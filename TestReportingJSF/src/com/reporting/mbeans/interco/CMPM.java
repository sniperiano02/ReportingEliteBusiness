...
public class CMPM {
	...
	private List<Object[]> resultatcmpm;
...
public void setChoix_operateur(String choix_operateur) {
	...
	public List<Object[]> getResultatcmpm() {
		return resultatcmpm;
	}
	public void setResultatcmpm(List<Object[]> resultatcmpm) {
		this.resultatcmpm = resultatcmpm;
	}
		...
		chartDisplayed = false;
				...
				if(compJourOutputHeure!=null){
					...
					if(this.getChoix_periode().equals("Par Jour")){
						...
						ValueExpression valjour = createValueExpression("#{cmpm.date_ParJourDeb}", Date.class);
						...
						ValueExpression valjourfin = createValueExpression("#{cmpm.date_ParJourFin}", Date.class);
					...
					ValueExpression valjour = createValueExpression("#{cmpm.date_Parheure}", Date.class);
					...
					AjaxBehavior ajax = new AjaxBehavior();
				       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
				        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
				            createActionMethodExpression("#{cmpm.handlechange1}"),createActionMethodExpression("#{cmpm.handlechange1}")));
				        ajax.setUpdate("@form");
...
//					        
				         ...
				         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{cmpm.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
						...
						lstDateYears.setId("lstDateYears");
						 ValueExpression valueExp = createValueExpression("#{cmpm.date_year_deb}", Integer.class);
						...
						lstDateYearsFin.setId("lstDateYearsFin");
						 ValueExpression valueExp1 = createValueExpression("#{cmpm.date_year_fin}", Integer.class);
					...
					ValueExpression valjour = createValueExpression("#{cmpm.date_mois_debut}", Date.class);
					...
					ValueExpression valjourfin = createValueExpression("#{cmpm.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 resultatcmpm = interco_service.getStatInterco("substring(trancheHoraire,1,2)", "SUM(montantConverti)/(SUM(dureeTotal)/60)", "", where_liste);
		
	 		 }else if(this.getChoix_periode().equals("Par An")){
	 			 resultatcmpm = interco_service.getStatInterco("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(montantConverti)/(SUM(dureeTotal)/60)", "", where_liste);
	 			
	 			
		 		 }else if(this.getChoix_periode().equals("Par Mois")){
		 			 resultatcmpm = interco_service.getStatInterco("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(montantConverti)/(SUM(dureeTotal)/60)", "",  where_liste);

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			 resultatcmpm = interco_service.getStatInterco("to_char(to_date(dateAppel,'YYMMDD'),'DD')", "SUM(montantConverti)/(SUM(dureeTotal)/60)", "",  where_liste);

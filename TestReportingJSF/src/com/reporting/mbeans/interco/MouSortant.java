...
@ManagedBean(name="mou_sortant")
@ViewScoped
public class MouSortant {
	@EJB
	...
	private List<Object[]> resultatmou_sortant;
	...
	public List<Object[]> getResultatmou_sortant() {
		return resultatmou_sortant;
	}
	public void setResultatmou_sortant(List<Object[]> resultatmou_sortant) {
		this.resultatmou_sortant = resultatmou_sortant;
	}
		...
		chartDisplayed = false;
				...
				if(compJourOutputHeure!=null){
					...
					if(this.getChoix_periode().equals("Par Jour")){
						...
						ValueExpression valjour = createValueExpression("#{mou_sortant.date_ParJourDeb}", Date.class);
						...
						ValueExpression valjourfin = createValueExpression("#{mou_sortant.date_ParJourFin}", Date.class);
					...
					ValueExpression valjour = createValueExpression("#{mou_sortant.date_Parheure}", Date.class);
					...
					AjaxBehavior ajax = new AjaxBehavior();
				       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
				        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
				            createActionMethodExpression("#{mou_sortant.handlechange1}"),createActionMethodExpression("#{mou_sortant.handlechange1}")));
				        ajax.setUpdate("@form");
...
//					        
				         ...
				         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{mou_sortant.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
						...
						lstDateYears.setId("lstDateYears");
						 ValueExpression valueExp = createValueExpression("#{mou_sortant.date_year_deb}", Integer.class);
						...
						lstDateYearsFin.setId("lstDateYearsFin");
						 ValueExpression valueExp1 = createValueExpression("#{mou_sortant.date_year_fin}", Integer.class);
					...
					ValueExpression valjour = createValueExpression("#{mou_sortant.date_mois_debut}", Date.class);
					...
					ValueExpression valjourfin = createValueExpression("#{mou_sortant.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		resultatmou_sortant = new ArrayList<>();
		if(this.getChoix_periode().equals(" ")){
			...
			where_liste.add("typeStat='produit'");
			if(this.getChoix_periode().equals("Par Jour")){
		...
		if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 resultatmou_sortant = interco_service.getStatInterco("substring(trancheHoraire,1,2)", "SUM(dureeTotal/60)/SUM(nombreTotal)", "", where_liste);
		
	 		 }else if(this.getChoix_periode().equals("Par An")){
	 			 resultatmou_sortant = interco_service.getStatInterco("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(dureeTotal/60)/SUM(nombreTotal)", "", where_liste);
	 			
	 			
		 		 }else if(this.getChoix_periode().equals("Par Mois")){
		 			 resultatmou_sortant = interco_service.getStatInterco("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')", "SUM(dureeTotal/60)/SUM(nombreTotal)", "",  where_liste);

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			 resultatmou_sortant = interco_service.getStatInterco("to_char(to_date(dateAppel,'YYMMDD'),'DD')", "SUM(dureeTotal/60)/SUM(nombreTotal)", "",  where_liste);

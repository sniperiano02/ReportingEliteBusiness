...
@ManagedBean(name="qualiteInterco")
@ViewScoped
public class QualiteIntercoBean {

	
	@EJB
	private StatCtiImplRemote statQualite;
	
	private String choix_periode;
	
	private List<Object[]> resultQualite;
	...
	@EJB
	private OperateurIntercoRemote operRemote;
...
public void init(){
		...
		listeoper=operRemote.getAllOperateurs("I");
		setChoix_oper(new OperateurInterco()); 
		liste_desAns=statQualite.getAllYears();
		...
		chartDisplayed = false;
				...
				if(compJourOutputHeure!=null){
					...
					if(this.getChoix_periode().equals("Par Jour")){
						...
						ValueExpression valjour = createValueExpression("#{qualiteInterco.date_ParJourDeb}", Date.class);
						...
						ValueExpression valjourfin = createValueExpression("#{qualiteInterco.date_ParJourFin}", Date.class);
					...
					ValueExpression valjour = createValueExpression("#{qualiteInterco.date_Parheure}", Date.class);
					...
					AjaxBehavior ajax = new AjaxBehavior();
				       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
				        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
				            createActionMethodExpression("#{qualiteInterco.handlechange1}"),createActionMethodExpression("#{qualiteInterco.handlechange1}")));
				        ajax.setUpdate("@form");
...
//					        
				         ...
				         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{qualiteInterco.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
						...
						lstDateYears.setId("lstDateYears");
						 ValueExpression valueExp = createValueExpression("#{qualiteInterco.date_year_deb}", Integer.class);
						...
						lstDateYearsFin.setId("lstDateYearsFin");
						 ValueExpression valueExp1 = createValueExpression("#{qualiteInterco.date_year_fin}", Integer.class);
					...
					ValueExpression valjour = createValueExpression("#{qualiteInterco.date_mois_debut}", Date.class);
					...
					ValueExpression valjourfin = createValueExpression("#{qualiteInterco.date_mois_fin}", Date.class);
...
SubTitle ="Par An entre "+deb+" et "+fin;
			...
			resultTaxe = statQualite.getAllQualite("", "", "", "", whereliste1);
		 ...
		 if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 resultQualite= removeDuplicate(generateList,statQualite.getAllStatQualite("substring(trancheHoraire,1,2)",where_liste));
	 		 }else if(this.getChoix_periode().equals("Par An")){
	// sc1.setMap(statINLoc.getMscByFilters("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(revenue)*60/SUM(duree)", "", "Group By Extract (year from to_date(dateAppel,'YYMMDD')) ", where_liste));	
	 resultQualite= statQualite.getAllStatQualite("Extract (year from to_date(dateAppel,'YYMMDD'))",where_liste);
		 		 }else if(this.getChoix_periode().equals("Par Mois")){
		//	 sc1.setMap(statINLoc.getMscByFilters("Extract (month from to_date(dateAppel,'YYMMDD'))", "SUM(revenue)*60/SUM(duree)", "", "Group By Extract (month from to_date(dateAppel,'YYMMDD')) ", where_liste));	
			 resultQualite= removeDuplicate(generateList,statQualite.getAllStatQualite("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY') ",where_liste));
		...
		// sc1.setMap(statINLoc.getMscByFilters("extract(day from  to_date(dateAppel,'YYMMDD'))", "SUM(revenue)*60/SUM(duree)", "", "Group By  extract(day from  to_date(dateAppel,'YYMMDD'))", where_liste));	
		 resultQualite= removeDuplicate(generateList,statQualite.getAllStatQualite("to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD')",where_liste));
...
chartDisplayed=true;
	...
	public List<Object[]> getResultQualite() {
		return resultQualite;
	}
	public void setResultQualite(List<Object[]> resultQualite) {
		this.resultQualite = resultQualite;
	}

...
@ManagedBean(name="rec_cti_sysfact")
@ViewScoped
public class RecCtiSysFactMbean implements Serializable {
	...
	@EJB
	private StatReconCtiSysFactRemote statRecon;
		...
		chartDisplayed = false;
				...
				if(compJourOutputHeure!=null){
					...
					if(this.getChoix_periode().equals("Par Jour")){
						...
						ValueExpression valjour = createValueExpression("#{rec_cti_sysfact.date_ParJourDeb}", Date.class);
						...
						ValueExpression valjourfin = createValueExpression("#{rec_cti_sysfact.date_ParJourFin}", Date.class);
					...
					ValueExpression valjour = createValueExpression("#{rec_cti_sysfact.date_Parheure}", Date.class);
					...
					AjaxBehavior ajax = new AjaxBehavior();
				       // MethodExpression me = elFactory.createMethodExpression(elContext, "#{sampleMBean.processAction}", null, new Class<?>[]{BehaviorEvent.class});
				        ajax.addAjaxBehaviorListener(new AjaxBehaviorListenerImpl(
				            createActionMethodExpression("#{rec_cti_sysfact.handlechange1}"),createActionMethodExpression("#{rec_cti_sysfact.handlechange1}")));
				        ajax.setUpdate("@form");
...
//					        
				         ...
				         MethodExpression me = ef.createMethodExpression(fc.getELContext(), "#{rec_cti_sysfact.handlechange2}", null, new Class<?>[]{BehaviorEvent.class});
						...
						lstDateYears.setId("lstDateYears");
						 ValueExpression valueExp = createValueExpression("#{rec_cti_sysfact.date_year_deb}", Integer.class);
						...
						lstDateYearsFin.setId("lstDateYearsFin");
						 ValueExpression valueExp1 = createValueExpression("#{rec_cti_sysfact.date_year_fin}", Integer.class);
					...
					ValueExpression valjour = createValueExpression("#{rec_cti_sysfact.date_mois_debut}", Date.class);
					...
					ValueExpression valjourfin = createValueExpression("#{rec_cti_sysfact.date_mois_fin}", Date.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			if(this.getChoix_periode().equals("Par Jour")){
					...
					resultCDRInexistant= statRecon.getAllStatReconCtiSysFact("to_char(to_date(dateAppel,'YYMMDD'),'YYYY-MM-DD')",where);
				...
				resultCDRInexistant= statRecon.getAllStatReconCtiSysFact("Extract (year from to_date(dateAppel,'YYMMDD'))",where);
		...
		resultCDRInexistant= statRecon.getAllStatReconCtiSysFact("to_char(to_date(dateAppel,'YYMMDD'),'MM/YYYY')",where);
...
//		System.out.println(deb);
	List<Object[]> result1 = new ArrayList<Object[]>();
		...
		if(this.getChoix_periode().equals("Par Jour")){
			...
			yDetail = ",SUM(nbAppelCti),SUM(dureeCti),SUM(nbAppelIntec) ,SUM(dureeIntec) ,SUM(nbAnomalie),SUM(dureeAnomalie)";
			resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconCtiSysFact("to_date(dateAppel,'YYMMDD')", where);
		...
		yDetail = ",SUM(nbAppelCti),SUM(dureeCti),SUM(nbAppelIntec) ,SUM(dureeIntec) ,SUM(nbAnomalie),SUM(dureeAnomalie)";
		resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconCtiSysFact(xDetail, where);
...
Subtitle1="Par Jour en "+selectedRecon[0].toString();
		...
		Details = statRecon.getDetailsStatReconCtiSysFact(xDetail,yDetail, where);

 ...
 */
@Stateless
public class StatIntecImpl implements StatIntecImplRemote {
	...
	public String getStatIntec() {
		...
		return em.find(StatIntecInter.class, 1).getDateAppel();
	}
	...
	public Date getIntecDate() {
		// TODO Auto-generated method stub
		return (Date) em.createQuery("SELECT to_date(dateAppel,'YYMMDD') from StatIntecInter s ").getSingleResult();
	}
...
return em.createQuery("Select to_date(dateAppel,'YYMMDD'),SUM(duree) from StatIntecInter s Group By to_date(dateAppel,'YYMMDD')  )").getResultList();
		}
...
return em.createQuery("Select to_date(dateAppel,'YYMMDD'),SUM(nbAppel) from StatIntecInter s Group By to_date(dateAppel,'YYMMDD')  )").getResultList();
		}
	...
	public List<Integer> getAllYears() {
		// TODO Auto-generated method stub
		return em.createQuery("Select distinct extract(year from to_date(dateAppel,'YYMMDD'))From StatIntecInter s order by extract(year from to_date(dateAppel,'YYMMDD')) ").getResultList();
	}
	...
	public List<String> getAlltrancheHoraire() {
		// TODO Auto-generated method stub
		return em.createQuery("Select distinct trancheHoraire from StatIntecInter s").getResultList();
	}
	...
	public List<Object[]> getAllStatIntec(String x, List<String> list_y,String table,
		...
		return em.createQuery("Select "+x+","+y+"From StatIntecInter s Where  "+where+" Group By "+x+ " Order By "+x ).getResultList();
	}
	...
	public List<Object[]> getAllStatIntecStatic(String x, String y,
		...
		if(x.contains("substring")){
			...
			Query q=  em.createQuery("Select "+x+","+y+"From StatIntecInter s Where  "+where+" Group By trancheHoraire Order By CAST (trancheHoraire as integer)" );
			res=q.getResultList();
			if(!res.isEmpty()){
				return res;
			}else
			...
			Query q=  em.createQuery("Select "+x+","+y+"From StatIntecInter s Where  "+where+" Group By "+x+" Order By "+x );
			res=q.getResultList();
			if(!res.isEmpty()){
				return res;
			}else
